...
function getToken({ id, email, version, scope, name }) {
  return { id, email, version, scope, name, token: jwt.sign({ id, email, version, scope }, config.jwt.password, config.jwt.signOptions), };
}
...
const oauthCallback = async (query) => {
  ...
  if (!user) {
    ...
    const toInsert = { ...baseUser, is_deleted: false, is_validated: true, version: 10 };
