...
function signToken({
  id, email, version = USER_RECORD_VERSION, scope
}) {
  return jwt.sign({
    id, email, version, scope
  }, config.jwt.password, config.jwt.signOptions);
}

function getUserWithToken({
  id, email, version, scope, name
}) {
  return {
    id,
    email,
...
async function login(email, password) {
  ...
  return getUserWithToken(userData);
}
...
async function validate(token) {
  ...
  return knex.transaction((trx) => {
    async function returnsAsync() {
      ...
      return getUserWithToken(queryResult[0]);
    }
    try {
      return returnsAsync();
    } catch (err) {
...
const oauthCallback = async (query) => {
  ...
  try {
    ...
    debug('error getting user by email', err);
  }
  if (!user) {
    ...
    const toInsert = {
      ...baseUser, is_deleted: false, is_validated: true, version: USER_RECORD_VERSION
    };
...
const oauthTokenLogin = async (id) => {
  ...
  return { user, token: getUserWithToken(omit(user, 'password')) };
};
